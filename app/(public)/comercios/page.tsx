...
export default async function ComerciosPage({ searchParams }: { searchParams: { segmentacao?: string; busca?: string; page?: string } }) {
  ...
  const [empresas, total, segmentacoes] = await Promise.all([
    prisma.empresa.findMany({
      ...
      skip: (page - 1) * limit,
      take: limit,
      include: { galeria: { orderBy: { ordem: 'asc' }, take: 5 }, promocoes: { where: { ativo: true } } },
    }),
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <BuscaComercios segmentacoes={segmentacoes} segmentacaoAtiva={segmentacao} buscaAtiva={busca} />
          ...
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
                p === page ? 'bg-primary text-white' : 'bg-white border border-border text-text hover:bg-primary-light'
              }`}
            >
              {p}
            </Link>
