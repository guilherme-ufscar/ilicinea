  ...
  try {
    ...
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-10">
          ...
          <form className="sm:ml-auto flex gap-2" action="/noticias" method="GET">
            {categoria && categoria !== 'Todas' && (
          ...
          <>
                  ...
                  <div className="relative w-full aspect-[21/9] bg-surface-soft">
                    {hero.imagemCapa ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center text-text-muted text-sm">
                        <span className="inline-flex items-center gap-2">Sem imagem de capa</span>
                      </div>
                    )}
                  </div>
                  <div className="p-6">
                    <div className="flex flex-wrap gap-2 mb-3">
                      {hero.categorias.map((cat) => (
                        <span key={cat} className="badge-essencial">
                          {cat}
                        </span>
                      ))}
                    </div>
                    ...
                    {hero.resumo && <p className="text-text-soft line-clamp-2 mb-3">{hero.resumo}</p>}
                  ...
                  >
                    <div className="relative w-full aspect-video bg-surface-soft">
                      {noticia.imagemCapa ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-text-muted text-sm">
                          <span className="inline-flex items-center gap-2">Sem imagem</span>
                        </div>
                      )}
                    </div>
                    <div className="p-5 flex flex-col flex-1">
                      <div className="flex flex-wrap gap-1.5 mb-2">
                        {noticia.categorias.slice(0, 3).map((cat) => (
                          <span key={cat} className="badge-essencial text-xs px-2 py-0.5">
                            {cat}
                          </span>
                        ))}
                      </div>
            ...
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-2">
                {page > 1 && (
                  <Link href={buildUrl({ page: String(page - 1) })} className="btn-outline text-sm py-1.5 px-3">
                    Anterior
                  </Link>
                )}
    ...
    return (
      <PublicModuleUnavailable
        title="Módulo de notícias temporariamente indisponível"
