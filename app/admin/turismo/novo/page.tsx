...
export default function NovoTurismoPage() {
  ...
  async function handleSubmit(e: React.FormEvent, publish = false) {
    e.preventDefault()
    setError(null)
    if (!form.nome || !form.resumo || !form.descricao) {
      setError('Nome, resumo e descrição são obrigatórios')
      return
    }
    setLoading(true)
    try {
      const res = await fetch('/api/turismo', {
        method: 'POST',
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-2xl font-bold text-text mb-6">Novo ponto turístico</h1>
